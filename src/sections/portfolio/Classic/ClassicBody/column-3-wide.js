...
export default function CLassicBody() {
  ...
  const [tag, setTag] = useState('all');
	...
	useEffect(
		() => {
			tag === 'all' ? setFilteredImages(images.col_3_wide) : setFilteredImages(images.col_3_wide.filter(image => image.tagi === tag));
		},
		[tag]
	);

  return <>
   <div className="portfolio-section grid--no-gap" id="portfolio-section">
  ...
  <div className="container-fluid">
    <div className="row justify-content-center">
      <div className="col-12">
        <div className="iso-mas-grid-wrap gx-0 row">
        {filteredImages.map(({image,id,tagi}) => (
						<div className={`col-lg-4  col-md-6  col-sm-6 gr-pb-0  ${tagi === tag | tag === "all" ?  "is-animated" :""} `}  key={id}>
            <Card image={image}/>
          </div>
					))}
